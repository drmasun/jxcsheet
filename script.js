...
const filters = [
	...
	document.getElementById('racetime'),
...
var construct = function(data, score) {
	...
	row.append(name, day, meet, race, course, grade, mile1, mile2, mile3);
	return row;
}
...
function process_wb(wb) {
	runnerdata = XLSX.utils.sheet_to_json(wb.Sheets[wb.SheetNames[0]], { raw: false, blankrows: false });
	//pending = false;
	return runnerdata;
}
...
function filterload(look, empty) {
	for (var d = 0; d < look.length; d += 1) {
		var dtalist = document.getElementById(`${look[d]}s`);
		var elmnts = dtalist.getElementsByTagName('option');
		for (var e = 0; 0 < elmnts.length; e += 1)
			dtalist.getElementsByTagName('option')[0].remove();
	}
	
	var indexes = Array(look.length).fill(0);
	var info = Array(look.length).fill(null).map(() => Array());
	var table = document.getElementsByTagName('tr');
	var categories = table[0].getElementsByTagName('td');
	for (var y = 0; y < look.length; y += 1)
		for (; indexes[y] < categories.length && categories[indexes[y]].innerText.toLowerCase() != look[y].toLowerCase(); indexes[y] += 1)
			continue;
	
	var loopnum = (empty) ? runnerdata.length : table.length;
	for (var w = 0; w < look.length; w += 1)
		for (var i = 1; i < loopnum; i += 1) {
			var temp = runnerdata[(empty) ? i : (Number.parseInt($(table[i]).attr('rownum'), 10) - 1)][_.capitalize(look[w])];
			if (info[w].includes(temp) || (!empty && table[i].style.display == "none")) continue;
			info[w].push(temp);
		}
		
	for (var a = 0; a < info.length; a += 1) {
		var filter = document.getElementById(`${look[a]}s`);
		for (var b = 0; b < info[a].length; b += 1) {
			var optn = document.createElement('option');
			optn.value = info[a][b];
			filter.appendChild(optn);
		}
	}
}
...
function sorttable(index, ascending) {
	var table = document.getElementsByTagName('tr');
	var category = table[0].getElementsByTagName('th')[index].innerText;
	while (1 < table.length)
		table[1].remove();
...
function afterload(rows) {
	console.log('Done!');
	for (var w of document.getElementsByTagName('input'))
		w.disabled = false;
...
var filtermatch = function(item) {
	...
	for (var i = 1; i < filters.length; i += 1)
		contentlength += filters[i].value.length;
	if (contentlength > 0) {
		var appears = true;
		for (var j = 1; j < filters.length && appears; j += 1)
			if (filters[j].value.length > 0)
				appears = filters[j].value.toLowerCase() == obj[columns[j]].toString().toLowerCase();
		return appears;
	} else return true;
}
...
var filtertable = function() {
	...
	for (var a = 1; a < filters.length; a += 1)
		contentlength += filters[a].value.length;
	while (1 < table.length)
		table[1].remove();
	datashown = [];
	for (var b = 0; b < (headers.length - 3); b += 1) {
		headers[b].classList.remove('ascending', 'descending');
		headers[b].setAttribute('data-active', 'false');
	}
	
	if (filters[0].value.length > 0) {
		var result = fuse.search({ 'Name': filters[0].value });
		console.log(result);
	...
	filterload(['meet', 'course', 'grade'], (filters[0].value.length < 1 && contentlength < 1));
}

document.getElementById('clear').addEventListener('click', function(e){
	for (var v of document.getElementsByTagName('input'))
		v.value = '';
	filtertable();
});

for (var k of filters)
	k.addEventListener('change', filtertable);
